...
// Map employee names to cashier_ids and filter to only cashiers and waitresses
$employeeNames = [];
$cashierOnlySales = [];
$allCashierIds = [];
...
foreach ($employeeSales as $employee) {
    ...
    // Check if this employee is a cashier or waitress
    $isCashierOrWaitress = false;
    
    // Fast check if we have the list
    if (!empty($allCashierIds) && isset($allCashierIds[$cashierId])) {
        $isCashierOrWaitress = true;
    } else {
        // Check individually
        try {
            ...
            $stmt->execute([$cashierId]);
            $user = $stmt->fetch(PDO::FETCH_ASSOC);
            if ($user && in_array(strtolower($user['role']), ['cashier', 'waitress'])) {
                $isCashierOrWaitress = true;
...
// Replace employeeSales with filtered cashier and waitress sales
...
<head>
    ...
    <title>Cash Up - Daily Sales by Cashier & Waitress</title>
...
<body class="bg-gray-50" style="background-color:rgb(249 250 251 / var(--tw-bg-opacity, 1))">
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <h3 class="text-xl font-bold text-gray-600">N$<?php echo number_format($grandTotalCash, 2); ?></h3>
                            </div>
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <h3 class="text-xl font-bold text-gray-600">N$<?php echo number_format($grandTotalEft, 2); ?></h3>
                            </div>
                            <div class="p-2 bg-gray-100 rounded-full">
                                ...
                                <div class="py-3 px-4 no-print">
                                    <div class="flex flex-col gap-3 md:flex-row md:gap-4 items-stretch md:items-center justify-between">
                                        <div class="relative w-full md:max-w-xs md:w-auto">
                                            ...
                                            <input type="text" id="searchInput" class="py-2.5 px-3 ps-9 block w-full border-gray-200 shadow-sm rounded-lg text-sm focus:z-10 focus:border-gray-500 focus:ring-gray-500" placeholder="Search cashiers & waitresses...">
                                        ...
                                        <div class="flex gap-2 items-stretch md:items-center">
                                            <select id="employeeFilter" class="flex-1 md:flex-none py-2.5 px-3 border border-gray-200 rounded-lg text-sm focus:border-gray-500 focus:ring-gray-500" onchange="updateFilters()">
                                                <option value="all" <?php echo $selectedEmployee === 'all' ? 'selected' : ''; ?>>All Cashiers & Waitresses</option>
                                            ...
                                            <?php if (empty($employeeSales)): ?>
                                                <tr>
                                                    <td colspan="5" class="px-6 py-12 text-center">
                                                        ...
                                                        <p class="text-gray-500 text-lg">No cashier or waitress sales found for the selected date.</p>
                                                    </td>
                                                </tr>
                                            <?php else: ?>
                                                        ...
                                                        <td class="px-6 py-4 whitespace-nowrap">
                                                            <div class="flex items-center">
                                                                <div class="h-10 w-10 flex-shrink-0 rounded-full bg-gray-100 flex items-center justify-center">
                                                                    <span class="text-gray-600 font-semibold text-sm"><?= strtoupper(substr($employeeName, 0, 2)) ?></span>
                                                                </div>
                                                                <div class="ml-4">
                                    ...
                                    <?php else: ?>
                                            ...
                                            <div class="mobile-cashier-card" 
                                                ...
                                                <div class="mobile-cashier-card-body">
                                                    <div class="mobile-cashier-card-field">
                                                        ...
                                                        <div class="mobile-cashier-card-value text-gray-600 font-semibold">
                                                            N$<?= number_format($employee['total_cash'], 2) ?>
                                                        </div>
                                                    </div>
                                                    <div class="mobile-cashier-card-field">
                                        ...
                                        <div class="mobile-cashier-card bg-gray-50 border-2 border-gray-300 mt-4">
                                            ...
                                            <div class="mobile-cashier-card-body">
                                                <div class="mobile-cashier-card-field">
                                                    <div class="mobile-cashier-card-label">Cash Sales</div>
                                                    <div class="mobile-cashier-card-value text-gray-600 font-bold">
                                ...
                                <div class="py-3 px-4 no-print">
                                    <div class="flex flex-col md:flex-row items-center justify-between gap-3 md:gap-4">
                                        <div class="text-xs md:text-sm text-gray-700 text-center md:text-left">
                                            Showing <span id="showingFrom" class="font-semibold">1</span> to <span id="showingTo" class="font-semibold"><?= count($employeeSales) ?></span> of <span id="totalRows" class="font-semibold"><?= count($employeeSales) ?></span> employees
                                        </div>
    ...
    <script>
        ...
        if (typeof sendToPrinter === 'undefined') {
            console.warn('[admin/cash-up.php] sendToPrinter not loaded from receipt.php, using fallback');
        ...
        function renderTable() {
            ...
            if (tableBody) {
                ...
                if (pageRows.length === 0) {
                    tableBody.innerHTML = '<tr><td colspan="5" class="px-6 py-12 text-center"><i data-lucide="users" class="w-16 h-16 text-gray-300 mx-auto mb-4"></i><p class="text-gray-500 text-lg">No cashiers or waitresses found matching your criteria.</p></td></tr>';
                } else {
            ...
            if (mobileContainer) {
                ...
                if (pageCards.length === 0) {
                    mobileContainer.innerHTML = '<div class="text-center py-12"><i data-lucide="users" class="w-16 h-16 text-gray-300 mx-auto mb-4"></i><p class="text-gray-500 text-lg">No cashiers or waitresses found matching your criteria.</p></div>';
                } else {
        ...
        function renderPagination(totalPages) {
            ...
            for (let i = 1; i <= totalPages; i++) {
                if (i === 1 || i === totalPages || (i >= currentPage - 2 && i <= currentPage + 2)) {
                    ...
                    pageButton.className = `min-w-[40px] min-h-[40px] flex justify-center items-center text-gray-800 hover:bg-gray-100 py-2.5 text-sm rounded-full ${i === currentPage ? 'bg-gray-100 text-gray-600 font-semibold' : ''}`;
        ...
        // Print all items sold receipt
        async function printAllItemsReceipt() {
            try {
                Swal.fire({
                    title: 'Loading...',
                    text: 'Fetching all items sold',
                    allowOutsideClick: false,
                    didOpen: () => {
                        Swal.showLoading();
                    }
                ...
                const response = await fetch(`get_all_items_sold.php?date=${encodeURIComponent(selectedDate)}`);
                ...
                // Prepare receipt data with all items
                const receiptData = {
                    is_cashup_report: true,
                    date: selectedDate,
                    cashier_username: 'All Cashiers',
                    items: data.items || [],
                    total_cash_sales: data.total_cash || 0,
                    total_eft_sales: data.total_eft || 0,
                    grand_total: data.grand_total || 0,
                    cash_sales: data.total_cash || 0,
                    eft_sales: data.total_eft || 0,
                    show_all_items: true
                };
                ...
                if (result.success) {
                    Swal.fire({
                        icon: 'success',
                        title: 'Receipt Printed',
                        text: 'All items sold receipt printed successfully!',
                        timer: 2000,
                        timerProgressBar: true,
                        showConfirmButton: false
                    });
