...
<main class="flex-1 p-6 bg-gray-50 flex">
    ...
    <div class="w-3/4 pr-6 max-h-[calc(100%)]">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 py-4 <?php echo count($products) > 6 ? 'overflow-y-auto pr-4 custom-scrollbar' : ''; ?>" id="productGrid" style="max-height: calc(100vh - 8rem);">
        <?php foreach ($products as $product): ?>
                <div class="bg-white rounded-xl shadow-lg overflow-hidden cursor-pointer transition-transform duration-300 hover:scale-105 product-item select-none" data-price="<?= $product['price'] ?>" data-name="<?= htmlspecialchars($product['name']) ?>" onclick="addToCart(this)">
                    <div class="w-full h-48">
                        <img src="products/<?= htmlspecialchars($product['image_url']) ?>" alt="<?= htmlspecialchars($product['name']) ?>" class="w-full h-full object-cover pointer-events-none">
                    </div>
