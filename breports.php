...
$bdWhereOrdersCreated = bdSingleDayWhereSql('orders.created_at', ':selectedDate', ':nextDay', $closingTime, $isAfterMidnight);
$bdWhereCreditSalesCreated = bdSingleDayWhereSql('credit_sales.created_at', ':selectedDate', ':nextDay', $closingTime, $isAfterMidnight);
...
$creditSalesQuery = $db->prepare("
    SELECT 
    SUM(CASE WHEN payment_status IN ('unpaid', 'partial') THEN total_amount ELSE 0 END) as total_issued,
    SUM(CASE WHEN payment_status = 'unpaid' THEN total_amount - paid_amount 
...
$paidCreditQuery = $db->prepare("
    ...
    FROM payments p
    JOIN credit_sales cs ON p.sale_id = cs.id
    WHERE ($bdWherePayment) AND cs.payment_status = 'paid'  -- Only include cash payments, not EFT
");
...
1. Initial cash deposits (cash-in transactions for all time)
2. Plus cash sales (orders paid with cash FOR ALL DAYS up to selected date)
3. Plus paid credit sales (credit that has been paid FOR ALL DAYS up to selected date)
4. Minus cash withdrawals (cash-out transactions for all time)
...
$creditQuery = $db->prepare("
    ...
    (SELECT MAX(payment_date) FROM payments WHERE sale_id = credit_sales.id) as payment_date
    FROM credit_sales
    ...
    WHERE (
        ...
        (
            ($bdWhereCreditSalesCreated) AND payment_status IN ('unpaid', 'partial')
        )
    OR (
        -- Show paid/eft credit sales only on their payment date
        payment_status IN ('paid', 'eft') AND credit_sales.id IN (
            ...
            <div class="container mx-auto p-6">
                ...
                <div class="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-6">
                    ...
                    <div class="flex flex-col md:flex-row md:items-end gap-4 w-full md:w-auto">
                        <form method="POST" action="" class="flex flex-col md:flex-row md:items-end gap-4 w-full md:w-auto" id="dateForm">
                            <div class="relative">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-4 xl:grid-cols-2 gap-6 mb-8">

<!-- Cash Sales Card -->
<div class="bg-teal-300 rounded-lg shadow-lg p-6 transform hover:scale-105 transition-transform duration-200">
    <div class="flex items-center justify-between mb-4">
        <div>
            <p class="text-sm font-medium text-teal-800">Cash Sales</p>
            <h3 class="text-2xl font-bold text-teal-800">N$<?= number_format($cashSalesDisplayTotal, 2) ?></h3>
        </div>
        <div class="p-2 bg-teal-300 rounded-full">
            <i class="fas fa-dollar-sign text-teal-800 text-lg"></i>
        </div>
    </div>
    <p class="text-sm text-teal-800">Cash transactions + cash credit payments</p>
</div>

<!-- EFT Payments Card -->
<div class="bg-purple-200 rounded-lg shadow-lg p-6 transform hover:scale-105 transition-transform duration-200">
    <div class="flex items-center justify-between mb-4">
        <div>
            <p class="text-sm font-medium text-purple-700">EFT Payments</p>
            <h3 class="text-2xl font-bold text-purple-700">N$<?= number_format($totalEftPayments, 2) ?></h3>
        </div>
        <div class="p-2 bg-purple-200 rounded-full">
            <i class="fas fa-credit-card text-purple-600 text-lg"></i>
        </div>
    </div>
    <p class="text-sm text-purple-600">Direct and credit EFT payments</p>
</div>
...
<!-- Total Revenue Card -->
<div class="bg-gray-300 rounded-lg shadow-lg p-6 transform hover:scale-105 transition-transform duration-200">
    <div class="flex items-center justify-between mb-4">
        <div>
            <p class="text-sm font-medium text-gray-600">Total Revenue</p>
            <h3 class="text-2xl font-bold text-gray-500">N$<?= number_format($totalCashOnHand, 2) ?></h3>
        </div>
        <div class="p-2 bg-gray-300 rounded-full">
            <i class="fas fa-wallet text-gray-600 text-lg"></i>
        </div>
    </div>
    <p class="text-sm text-gray-600">All sales (Cash, EFT, Credit)</p>
</div>
</div>
                    ...
                    ?>
                        <tr class="hover:bg-gray-50 transition-colors h-6">
                            ...
                            <td class="py-1 px-3 text-sm text-gray-600 truncate max-w-xs" title="<?= htmlspecialchars($row['products']) . ($row['creditor_name'] ? ' (' . htmlspecialchars($row['creditor_name']) . ')' : '') ?>">
                                ...
                                <span class="italic text-xs text-gray-500">(<?= htmlspecialchars($row['creditor_name']) ?>)</span>
                                <?php endif; ?>
                            </td>
                            <td class="py-1 px-3 text-sm text-gray-500">
                                <?php
...
<?php endif; ?>
    ...
    document.getElementById('printCashup').addEventListener('click', function() {
        ...
        .then(data => {
            ...
            // Get values from the stats cards - using better number parsing
            ...
            try {
                // Parse cash sales (from teal card)
                const cashSalesElement = document.querySelector('.bg-teal-300 h3');
                if (cashSalesElement) {
                    cashSalesTotal = parseFloat(cashSalesElement.textContent.replace('N$', '').replace(/,/g, '')) || 0;
                }
                
                // Parse EFT payments (from purple card)
                const eftSalesElement = document.querySelector('.bg-purple-200 h3');
                if (eftSalesElement) {
                    eftSalesTotal = parseFloat(eftSalesElement.textContent.replace('N$', '').replace(/,/g, '')) || 0;
                }
                
                // Parse unpaid credit (from yellow card)
                const unpaidCreditElement = document.querySelector('.bg-yellow-400 h3');
                if (unpaidCreditElement) {
                    unpaidCredit = parseFloat(unpaidCreditElement.textContent.replace('N$', '').replace(/,/g, '')) || 0;
                }
                
                // Parse total revenue (from gray card)
                const cashOnHandElement = document.querySelector('.bg-gray-300 h3');
                if (cashOnHandElement) {
                    cashOnHand = parseFloat(cashOnHandElement.textContent.replace('N$', '').replace(/,/g, '')) || 0;
                }
            } catch (error) {
                console.error('Error parsing card values:', error);
                alert('Error reading values from dashboard. Please try again.');
                return;
            }
            ...
            console.log('Cashup data prepared:', cashupData);
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Add change event listener to the date select
        const dateSelect = document.getElementById('date');
        if (dateSelect) {
