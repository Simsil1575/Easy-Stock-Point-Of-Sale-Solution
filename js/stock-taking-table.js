(function () {
    ...
    function showLoadingRow() {
        tableBody.innerHTML = '<tr><td colspan="9" class="py-8 px-6 text-center text-gray-500">Loading products...</td></tr>';
    }

    async function loadStockTakingPage(page) {
        if (isLoading) {
            return;
        }
        if (typeof window.onStockTakingTableBeforeLoad === 'function') {
            window.onStockTakingTableBeforeLoad();
        }
        isLoading = true;
        ...
        try {
            const response = await fetch('stock_taking_list.php?' + params.toString(), {
            ...
            tableBody.innerHTML = data.html || '<tr><td colspan="9" class="py-6 px-6 text-center text-gray-500">No products found</td></tr>';
            totalPages = data.total_pages || 1;
            totalItems = data.total || 0;
            currentPage = data.page || currentPage;
            updatePageDisplay();

            if (typeof window.onStockTakingTableAfterLoad === 'function') {
                window.onStockTakingTableAfterLoad(data);
            }
        } catch (error) {
            console.error(error);
            tableBody.innerHTML = '<tr><td colspan="9" class="py-6 px-6 text-center text-red-500">Failed to load products. Please refresh.</td></tr>';
    ...
    function scheduleReload(page) {
        ...
        fetchTimer = setTimeout(() => loadStockTakingPage(page || 1), 250);
    }
    ...
    function handlePrevPage() {
        if (!showAllMode && currentPage > 1) {
            loadStockTakingPage(currentPage - 1);
        }
    }
    ...
    function handleLastPage() {
        if (!showAllMode) {
            loadStockTakingPage(totalPages);
        }
    }
    ...
    function handlePageInput(inputElement) {
        ...
        if (!isNaN(desiredPage) && !showAllMode) {
            loadStockTakingPage(Math.min(Math.max(1, desiredPage), totalPages));
        }
    }
    ...
    if (viewAllBtn) {
        viewAllBtn.addEventListener('click', () => window.showAllRows());
    }
    ...
    window.reloadStockTakingTable = function (page) {
        loadStockTakingPage(page || currentPage);
    };

    window.initStockTakingTable = async function () {
        if (typeof window.bootStockTakingPage === 'function') {
            await window.bootStockTakingPage();
        } else {
            await loadStockTakingPage(1);
        }
    };

    window.initStockTakingTable();
})();
