...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['delete_item_id'])) {
        ...
        if ($paymentMethod === 'eft' && empty($transactionRef)) {
            $_SESSION['error'] = 'Transaction reference is required for EFT payments';
            header('Location: view-tab.php?id=' . $tabId);
            exit();
        }
        
        // Check tab balance
        $tabStmt = $db->prepare("SELECT current_balance FROM tabs WHERE id = ?");
...
@media (max-width: 768px) {
    ...
    .mobile-tab-card-value {
        font-size: 0.875rem;
        color: #111827;
        font-weight: 500;
    }
    
    .mobile-tab-card-balance {
        font-size: 1.125rem;
        font-weight: 700;
    }
    
    .mobile-tab-card-actions {
        display: flex;
        gap: 0.5rem;
        padding-top: 0.75rem;
    ...
    .mobile-tab-card-actions a,
    .mobile-tab-card-actions button {
        flex: 1;
        min-width: 70px;
    ...
    .mobile-tab-card-actions a i,
    .mobile-tab-card-actions button i {
        width: 18px;
        height: 18px;
    }
    
    .mobile-tab-card-full-width {
        grid-column: 1 / -1;
    }
...
<body style="background-color:rgb(249 250 251 / var(--tw-bg-opacity, 1))">
                            ...
                            <div class="border rounded-lg divide-y divide-gray-200 dark:border-gray-700 dark:divide-gray-700 bg-white">
                                ...
                                <div class="py-3 px-4">
                                    <div class="flex flex-col gap-3 md:flex-row md:gap-4 items-stretch md:items-center justify-between">
                                        <div class="relative w-full md:max-w-xs md:w-auto">
                                            ...
                                            <input type="text" id="hs-table-with-pagination-search" class="py-2.5 px-3 ps-9 block w-full border-gray-200 shadow-sm rounded-lg text-sm focus:z-10 focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none dark:bg-slate-900 dark:border-gray-700 dark:text-gray-400 dark:focus:ring-gray-600" placeholder="Search tabs...">
                                        ...
                                        <div class="flex gap-2 items-stretch md:items-center">
                                            <select id="statusFilter" class="flex-1 md:flex-none py-2.5 px-3 border border-gray-200 rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500">
                                    ...
                                    <?php else: ?>
                                        ...
                                        ?>
                                            <div class="mobile-tab-card<?= tab_is_marked_for_void($tab) ? ' border-red-300 bg-red-50' : '' ?>" 
                                                 ...
                                                 data-tab-creditor="<?= htmlspecialchars(strtolower($tab['creditor_name'] ?? '')) ?>"
                                                 data-tab-balance="<?= $tab['current_balance'] ?>"
                                                 onclick="handleMobileCardClick(event, <?= $tab['id'] ?>)">
                                                <div class="mobile-tab-card-header">
                                                    <div class="mobile-tab-card-title"><?= htmlspecialchars($tab['tab_name']) ?></div>
                                                    <div class="mobile-tab-card-id">ID: <?= $tab['id'] ?></div>
                                                </div>
                                                <div class="mobile-tab-card-body">
                                                    ...
                                                    <div class="mobile-tab-card-field">
                                                        <div class="mobile-tab-card-label">Balance</div>
                                                        <div class="mobile-tab-card-value mobile-tab-card-balance <?= $balanceColor ?>">
                                                            N$<?= number_format($tab['current_balance'], 2) ?>
                                                        </div>
                                                    </div>
                                                    <div class="mobile-tab-card-field">
                                                        <div class="mobile-tab-card-label">Creditor</div>
                                                        <div class="mobile-tab-card-value"><?= htmlspecialchars($tab['creditor_name'] ?? 'N/A') ?></div>
                                                    </div>
                                                    <div class="mobile-tab-card-field">
                                                        <div class="mobile-tab-card-label">Opened By</div>
                                                        <div class="mobile-tab-card-value"><?= htmlspecialchars($tab['opened_by_username'] ?? 'Unknown') ?></div>
                                                    </div>
                                                    <div class="mobile-tab-card-field mobile-tab-card-full-width">
                                                        <div class="mobile-tab-card-label">Opened At</div>
                                                        <div class="mobile-tab-card-value"><?= date('Y-m-d H:i', strtotime($tab['opened_at'])) ?></div>
                                                    </div>
                                                </div>
                                                    ...
                                                    <?php if ($tab['current_balance'] > 0): ?>
                                                        ...
                                                        <a href="view-tab.php?id=<?= $tab['id'] ?>&pay_all=1&amount=<?= number_format($tab['current_balance'], 2, '.', '') ?>"
                                                           class="bg-green-50 text-green-600 hover:bg-green-100">
                                                            <i data-lucide="credit-card" class="w-4 h-4 mx-auto"></i>
                                                            <span class="block mt-1">Pay</span>
                                                        </a>
                                                    <?php endif; ?>
                                                    <?php if ($tab['status'] === 'closed'): ?>
                                                        <form method="POST" style="display: inline; flex: 1;" 
                                                            ...
                                                            <button type="submit" 
                                                                ...
                                                                <i data-lucide="unlock" class="w-4 h-4 mx-auto"></i>
                                                                <span class="block mt-1">Reopen</span>
                                                            </button>
                                                        </form>
                                                    <?php endif; ?>
                                                    <?php if ($tab['current_balance'] == 0): ?>
                                ...
                                <div class="py-3 px-4">
                                    <div class="flex flex-col md:flex-row items-center justify-between gap-3 md:gap-4">
                                        <div class="text-xs md:text-sm text-gray-700 dark:text-gray-300 text-center md:text-left">
                                            Showing <span id="showingFrom" class="font-semibold">1</span> to <span id="showingTo" class="font-semibold">10</span> of <span id="totalRows" class="font-semibold"><?= count($tabs) ?></span> entries
                                        </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (mobileContainer) {
                allMobileCards = Array.from(mobileContainer.querySelectorAll('.mobile-tab-card'));
        ...
        function filterTable() {
            ...
            const filterItem = (item) => {
                ...
                const tabOpenedBy = item.getAttribute('data-tab-opened-by') || '';
                const tabOpenedAt = item.getAttribute('data-tab-opened-at') || '';
        ...
        function sortTable(columnIndex) {
            ...
            // Sort function for both rows and cards
            const sortItems = (a, b) => {
                let aValue, bValue;
        ...
        function renderTable() {
            ...
            if (mobileContainer) {
                ...
                if (pageCards.length === 0) {
                    mobileContainer.innerHTML = '<div class="text-center py-12"><i data-lucide="file-x" class="w-16 h-16 text-gray-300 mx-auto mb-4"></i><p class="text-gray-500 text-lg">No tabs found matching your criteria.</p></div>';
                } else {
                    pageCards.forEach(card => {
                        // Clone the card to avoid duplicate references
        ...
        function renderPagination(totalPages) {
            ...
            prevButton.className = 'p-2.5 md:p-2.5 min-w-[44px] min-h-[44px] md:min-w-[40px] md:min-h-[40px] inline-flex items-center justify-center gap-x-2 text-sm rounded-full text-gray-800 hover:bg-gray-100 disabled:opacity-50 disabled:pointer-events-none dark:text-white dark:hover:bg-white/10 dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600';
            ...
            for (let i = 1; i <= totalPages; i++) {
                if (i === 1 || i === totalPages || (i >= currentPage - 2 && i <= currentPage + 2)) {
                    ...
                    pageButton.className = `min-w-[44px] min-h-[44px] md:min-w-[40px] md:min-h-[40px] flex justify-center items-center text-gray-800 hover:bg-gray-100 py-2.5 text-sm rounded-full disabled:opacity-50 disabled:pointer-events-none dark:text-white dark:hover:bg-white/10 ${i === currentPage ? 'bg-gray-100 dark:bg-white/10 font-semibold' : ''}`;
            ...
            nextButton.className = 'p-2.5 md:p-2.5 min-w-[44px] min-h-[44px] md:min-w-[40px] md:min-h-[40px] inline-flex items-center justify-center gap-x-2 text-sm rounded-full text-gray-800 hover:bg-gray-100 disabled:opacity-50 disabled:pointer-events-none dark:text-white dark:hover:bg-white/10 dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600';
        ...
        function handleMobileCardClick(event, tabId) {
            // Don't navigate if clicking on action buttons or forms
            if (event.target.closest('a, button, form, .mobile-tab-card-actions')) {
                return;
            }
            window.location.href = `view-tab.php?id=${tabId}`;
        }
        ...
        function printTabBalance(tabId, tabName, creditorName, balance) {
            ...
            if (/^[0-9]$/.test(event.key)) {
                ...
                barcodeTimeout = setTimeout(() => {
                    ...
                    if (barcodeBuffer.length > 0) {
                        // Look for tab row or card with this ID (check both visible and all items)
                        const tabRow = document.querySelector(`.tab-row[data-tab-id="${barcodeBuffer}"]`);
                        const tabCard = document.querySelector(`.mobile-tab-card[data-tab-id="${barcodeBuffer}"]`);
                        if (tabRow || tabCard) {
                            // Get the tab ID from the row or card
                            const tabId = tabRow ? tabRow.getAttribute('data-tab-id') : tabCard.getAttribute('data-tab-id');
                            // Redirect to tab view page
                            window.location.href = `view-tab.php?id=${tabId}`;
                        } else {
                            // Check if tab exists in allRows or allMobileCards (might be filtered out)
                            const tabExists = allRows.some(row => row.getAttribute('data-tab-id') === barcodeBuffer) ||
                                            allMobileCards.some(card => card.getAttribute('data-tab-id') === barcodeBuffer);
                            if (tabExists) {
                                ...
                                // Find and scroll to the row or card
                                setTimeout(() => {
                                    const tabRow = document.querySelector(`.tab-row[data-tab-id="${barcodeBuffer}"]`);
                                    const tabCard = document.querySelector(`.mobile-tab-card[data-tab-id="${barcodeBuffer}"]`);
                                    const item = tabRow || tabCard;
                                    if (item) {
                                        item.scrollIntoView({ behavior: 'smooth', block: 'center' });
                                        item.classList.add('bg-blue-50');
                                        setTimeout(() => item.classList.remove('bg-blue-50'), 2000);
                                    }
                                }, 100);
                            } else {
                ...
                if (barcodeBuffer.length > 0) {
                    ...
                    if (tabRow || tabCard) {
                        ...
                        // Check if tab exists in allRows or allMobileCards
                        const tabExists = allRows.some(row => row.getAttribute('data-tab-id') === barcodeBuffer) ||
                                        allMobileCards.some(card => card.getAttribute('data-tab-id') === barcodeBuffer);
                        if (tabExists) {
                            // Clear filters and show the tab
