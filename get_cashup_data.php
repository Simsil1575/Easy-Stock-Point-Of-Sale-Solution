...
$allowedRoles = ['admin', 'manager', 'cashier', 'waitress'];
...
// so cash in till matches manager/get_cashup_data.php and cash.php for the same date
...
// Helper function for datetime range WHERE clause (inclusive start and end) - same as manager
...
try {
    ...
    // 1. Cash Sales (Expected) - orders.cashier_id is INTEGER; filter by id or username (same as manager logic)
    ...
    // Cash Out (ALL withdrawals – same as manager so cash in till balances)
    ...
    // 2. Card Sales (Expected) - eft_payments.cashier_id is INTEGER; filter by id or username (same as manager)
    $cardSalesExpected = 0;
    if ($eftTableExists) {
    ...
    try {
        ...
        // If breakdown fails, leave zeros
    }

    // Hansa Draught: total value, units, and split into cash vs EFT (same as manager)
    $hansaTotal = 0;
    $hansaUnits = 0;
    $hansaCash = 0;
    $hansaEft = 0;
    ...
    // 11. Total Value of Items Sold (same cashier filter as cash sales - orders)
