...
if (isset($orderData['is_cashup_report']) && $orderData['is_cashup_report']) {
    try {
        ...
        $printer->setEmphasis(true);
        $printer->text(sprintf("%-14s N$%8.2f\n", "EXPECTED CASH:", $calculatedExpectedCash));
        $printer->setEmphasis(false);
        
        // Add actual cash and difference if provided
        if (isset($orderData['actual_cash_in_till'])) {
            $printer->text(sprintf("%-14s N$%8.2f\n", "ACTUAL CASH:", $orderData['actual_cash_in_till']));
        ...
        if (isset($orderData['total_income'])) {
            ...
            $printer->setEmphasis(false);
            $printer->text(str_repeat('-', 42) . "\n");
            $printer->text(sprintf("%-20s N$%8.2f\n", "CASH:", $orderData['cash_sales']+ $orderData['credit_cash'] ?? 0));
            $printer->text(sprintf("%-20s N$%8.2f\n", "EFT:", $orderData['credit_eft'] + $orderData['eft_sales'] ?? 0));
