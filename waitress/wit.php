...
<body style="background-color:rgb(249 250 251 / var(--tw-bg-opacity, 1))">
    ...
    <div id="cart" class="w-full lg:w-96 h-full bg-gray-100 shadow-lg rounded-xl p-4 m-2 border border-gray-300 flex flex-col custom-scrollbar" style="height: calc(100vh - 4rem); overflow-y: auto; flex-shrink: 0; min-width: 0;">
        ...
        <div class="mt-4">
            <button id="addToTabButtonMain" class="bg-gray-700 hover:bg-gray-800 text-white px-4 py-3 rounded-lg shadow-md transition-colors duration-300 flex justify-center items-center text-lg w-full hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed" onclick="handleAddToTab()" disabled>
        ...
        <script>
            document.addEventListener('keydown', function(event) {
                if (event.key === 'Enter') {
                    ...
                    // Don't trigger add to tab if we're in the search bar (for barcode scanning)
                    if (document.activeElement === searchBar) {
                        return; // Let the handleBarcodeEntry function handle this
                    }
                    
                    // Trigger add to tab if focus is inside the cart area and cart has items
                    if (cartElement.contains(document.activeElement) || document.activeElement === cartElement) {
...
<script>
        ...
        if (typeof sendToPrinter === 'undefined') {
            console.warn('[waitress/home.php] sendToPrinter not loaded from receipt.php, using fallback');
        ...
        function updateCart() {
            ...
            // Enable/disable main Add to Tab button based on cart contents
            const addToTabButtonMain = document.getElementById('addToTabButtonMain');
            
            if (totalItems > 0) {
                // Enable main Add to Tab button
                if (addToTabButtonMain) {
                    addToTabButtonMain.disabled = false;
                    addToTabButtonMain.classList.remove('opacity-50', 'cursor-not-allowed');
                }
            } else {
                // Disable main Add to Tab button
                if (addToTabButtonMain) {
                    addToTabButtonMain.disabled = true;
                ...
                if (result.isConfirmed) {
                    ...
                    Swal.fire({
                        ...
                        try {
                            ...
                            // Fallback to document query if getContainer fails
