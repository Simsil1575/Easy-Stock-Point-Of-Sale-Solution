...
$isWaitressViewTab = strtolower(trim((string) ($_SESSION['role'] ?? ''))) === 'waitress';
$waitressCanTakeTabPayments = waitress_can_take_tab_payments($db);
$waitressTabPaymentsBlocked = $isWaitressViewTab && !$waitressCanTakeTabPayments;

function rejectInvalidManagerVoidPin(int $tabId = 0): void
{
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['delete_item_id'])) {
        ...
        $tabId = intval($_POST['tab_id']);
        if ($isWaitressViewTab) {
            $_SESSION['error'] = 'Waitresses cannot transfer tabs to credit sales. Ask a cashier or manager.';
            header('Location: view-tab.php?id=' . $tabId);
            exit();
        }
        ...
        if (!verifyManagerVoidPin($pin)) {
            rejectInvalidManagerVoidPin($tabId);
        }
        ...
        $tabId = intval($_POST['tab_id']);
        if ($waitressTabPaymentsBlocked) {
            $_SESSION['error'] = 'Waitresses cannot take tab payments. Ask a cashier or manager, or enable this under Admin → Business Settings.';
            header('Location: view-tab.php?id=' . $tabId);
            exit();
        }
...
<body class="bg-gray-50">
                    ...
                    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-3 sm:p-4 mb-6">
                        <div class="tab-key-info-row">
                            ...
                            <div class="flex flex-nowrap items-center gap-1.5 shrink-0 ml-auto">
                                ...
                                <?php endif; ?>
                                <?php if (!$waitressTabPaymentsBlocked && $viewTab['current_balance'] > 0.01): ?>
                                ...
                                <?php endif; ?>
                                <?php endif; ?>
                                <?php if (!$isWaitressViewTab && $viewTab['current_balance'] > 0): ?>
    ...
    <?php if (!$waitressTabPaymentsBlocked): ?>
    <!-- Payment Modal -->
                    ...
                    <div class="p-6 space-y-4">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div class="min-w-0">
                                ...
                                <p class="text-xs text-gray-500 mt-1"><?= $tabPrepaid > 0.01 ? ' · Prepaid on tab: N$' . number_format($tabPrepaid, 2) : '' ?></p>
                            </div>
    ...
    <script>
        ...
        const waitressCanPayTab = <?= $waitressTabPaymentsBlocked ? 'false' : 'true' ?>;
        ...
        const waitressCanTransferTab = <?= $isWaitressViewTab ? 'false' : 'true' ?>;
        const waitressCanAddTips = <?= $isWaitressViewTab ? 'false' : 'true' ?>;
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (waitressCanPayTab && urlParams.get('pay_all') === '1') {
            ...
            if (waitressCanPayTab && urlParams.get('openpay') === '1') {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    Swal.fire({
                        ...
                        promptManagerVoidPin({
                            title: 'Transfer to credit sale',
                            text: 'Enter manager void PIN to transfer this tab to a credit sale.',
                            confirmButtonText: 'Transfer',
                            confirmButtonColor: '#4F46E5'
                        }).then((pinResult) => {
                            if (!pinResult.isConfirmed) {
        ...
        function openVoidTabModal(tabId, tabName) {
            Swal.fire({
                ...
                promptManagerVoidPin({
                    title: 'Void tab',
                    text: 'Enter manager void PIN to void this tab.',
                    confirmButtonText: 'Void tab'
                }).then((pinResult) => {
                    if (!pinResult.isConfirmed) {
                        return;
                    }
