...
// Fetch the show_all_products and enforce_stock_checks settings
$settingStmt = $db->query("SELECT show_all_products, default_print_receipt, enforce_stock_checks FROM product_settings LIMIT 1");
$setting = $settingStmt->fetch(PDO::FETCH_ASSOC);
$show_all_products = $setting['show_all_products'] ?? 0; // Default to 0 if not set
$enforce_stock_checks = $setting['enforce_stock_checks'] ?? 0; // Default to 0 (disabled) if not set
...
<head>
    ...
    * {
    ...
    /* Ensure modals (SweetAlert2) always appear above sidebar */
    .swal2-container {
    ...
    @media (max-width: 1023px) {
        ...
        /* Ensure modals appear above sidebar and cart on mobile */
        .swal2-container {
            z-index: 10002 !important;
        }
    ...
    /* Tablet product grid - maintain aspect ratio and consistent sizing */
    ...
    /* Base product item styles - ensure consistent sizing across all devices */
    .product-item {
        display: flex;
        flex-direction: column;
        width: 100%;
...
<main class="flex-1 pr-4 p-6 bg-gray-50 flex flex-col lg:flex-row">
    ...
    <div class="w-full lg:w-3/4 pr-0 lg:pr-6 max-h-[calc(100%)]">
        ...
        <div class="mb-6 flex items-center justify-between gap-4">
            ...
            <div class="relative ml-auto">
                <div class="cursor-pointer" style="padding: 4px;">
                    ...
                    <?php $notificationCount = count($lowStock); ?>
        ...
        <?php else: ?>
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-4 md:gap-6 lg:gap-8 py-4 overflow-y-auto pr-0 lg:pr-4 custom-scrollbar" id="productGrid" style="max-height: calc(100vh - 8rem); height: auto; -webkit-overflow-scrolling: touch; overscroll-behavior: contain;">
    ...
    <div id="cart" class="w-full lg:w-96 h-full bg-gray-100 shadow-lg rounded-xl p-4 m-2 border border-gray-300 flex flex-col custom-scrollbar" style="height: calc(100vh - 4rem); overflow-y: auto;">
        ...
        <div class="flex items-center justify-between mb-4 text-xs">
            ...
            <div class="inline-flex rounded-full bg-gray-200 p-0.5">
                <button type="button"
                        id="orderTypeDineIn"
                        class="px-3 py-1 rounded-full text-[11px] font-semibold text-gray-700 bg-white shadow-sm border border-gray-300"
                        onclick="setOrderType('dine-in')">
                    Dine-in
                </button>
                <button type="button"
                        id="orderTypeTakeaway"
                        class="px-3 py-1 rounded-full text-[11px] font-medium text-gray-600 hover:text-gray-800 hover:bg-white/60 transition-colors"
        ...
        <div class="mt-4">
            ...
            <div id="extraButtonsContainer" class="hidden flex flex-wrap space-x-2 mb-4">
                ...
                <button class="bg-teal-700 text-gray-100 font-semibold px-3 py-2 rounded-lg shadow hover:bg-teal-800 transition-colors duration-200 mb-2 text-sm" onclick="handleOpenTill()">
                    <i data-lucide="wallet" class="w-4 h-4 inline-block mr-1 text-gray-100 opacity-80"></i>
                     Change
                </button>
            </div>
        </div>
...
<script>
        ...
        const defaultPrintReceipt = <?php echo $default_print_receipt ? 'true' : 'false'; ?>;
        ...
        // Stock check enforcement setting from PHP
        const enforceStockChecks = <?php echo $enforce_stock_checks ? 'true' : 'false'; ?>;
        ...
        function setOrderType(type) {
            ...
            const takeBtn = document.getElementById('orderTypeTakeaway');
            if (!dineBtn || !takeBtn) return;

            if (orderType === 'dine-in') {
                dineBtn.classList.add('bg-white', 'shadow-sm', 'border', 'border-gray-300', 'font-semibold');
                dineBtn.classList.remove('hover:bg-white/60');
                takeBtn.classList.remove('bg-white', 'shadow-sm', 'border', 'border-gray-300', 'font-semibold');
            } else {
                takeBtn.classList.add('bg-white', 'shadow-sm', 'border', 'border-gray-300', 'font-semibold');
                dineBtn.classList.remove('bg-white', 'shadow-sm', 'border', 'border-gray-300', 'font-semibold');
            }
        }
        ...
        function editQuantity(index) {
            ...
            function finishEditing() {
                ...
                if (newQuantity && newQuantity > 0) {
                    // Check stock if enforcement is enabled
                    if (enforceStockChecks) {
                        const productElement = document.querySelector(`.product-item[data-name="${cart[index].name}"]`);
                        if (productElement) {
                            const category = productElement.getAttribute('data-category')?.toLowerCase().trim();
            ...
            openCashDrawer().then((result) => {
                if (result.success) {
                    ...
                    Swal.fire({
                        icon: 'error',
                        title: 'Error',
                        text: result.message || 'Failed to open cash drawer',
                        timer: 2000,
                        showConfirmButton: false
                    });
                }
    ...
    function removeFromCart(index) {
        ...
        function handleEWalletPurchase() {
            ...
            // Check for out-of-stock items if enforcement is enabled
            if (enforceStockChecks) {
                const outOfStockItems = cart.filter(item => {
                    ...
                    if (productElement) {
                        ...
                        // Skip stock check for Food category products
                        if (isFood) {
                            return false;
                        }
                    ...
                    Swal.fire({
                        ...
                        footer: `
                            <div style="display: flex; justify-content: center; align-items: center;">
                                ...
                                <input type='checkbox' id='printReceiptCheckbox' ${defaultPrintReceipt ? 'checked' : ''} style='transform: scale(1.2); margin-right: 8px; vertical-align: middle;'>
                ...
                Swal.fire({
                    ...
                    .then(result => {
                        if (result.success) {
                            ...
                            cashSound.play();
                            // Kitchen ticket for mixed payments
                            printKitchenTicket({
                                items: saleData.items,
                                order_id: saleData.order_id,
                                cashier_username: saleData.cashier_username,
        ...
        // Send a simple kitchen order ticket (KOT) to the printer.
        function printKitchenTicket(payload) {
            if (!payload || !Array.isArray(payload.items) || payload.items.length === 0) {
                return;
            }
            try {
                fetch('kitchen_ticket.php', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
                    body: JSON.stringify(payload),
                }).catch(err => console.error('Kitchen ticket error:', err));
            } catch (e) {
                console.error('Kitchen ticket exception:', e);
            }
        }
    ...
    Swal.fire({
        ...
        .then(result => {
            if (result.success) {
                ...
                openCashDrawer();
                cashSound.play();
                // Kitchen ticket for cash orders
                printKitchenTicket({
                    items: data.items,
        ...
        fetch('get_product_quantities.php')
            ...
            .then(data => {
                data.forEach(product => {
                    ...
                    if (productElement) {
                        // Update quantity (always hidden)
                        const quantityElement = productElement.querySelector('p:last-child');
                        if (quantityElement && quantityElement.textContent.includes('Available:')) {
                            quantityElement.textContent = `Available: ${product.quantity}`;
                            quantityElement.className = `text-sm mb-2 hidden ${
                        ...
                        if (priceContainer) {
                            ...
                            if (discount > 0 && discountStart && discountEnd) {
                                ...
                                if (now >= start && now <= end) {
                                    ...
                                    const quantityHtml = `<p class="text-sm mb-2 hidden ${product.quantity < 5 ? 'text-red-600' : product.quantity < 10 ? 'text-yellow-600' : 'text-teal-600'}">Available: ${product.quantity}</p>`;
            ...
            Swal.fire({
                ...
                .then(result => {
                    if (result.success) {
                        ...
                        cashSound.play();
                        // Kitchen ticket for tab (dine-in) orders
                        printKitchenTicket({
                            items: saleData.items,
                            table_id: tableId,
                            table_name: tableName,
                        ...
                        // Only print kitchen ticket, no receipt for tab sales
                        clearCart();
                        refreshProductQuantities();
                        closeMobileCart();
                        Swal.fire({icon:'success', title:'Tab Sale Recorded', text: `Added to ${tableName}`, timer:1200, showConfirmButton:false});
                    } else {
