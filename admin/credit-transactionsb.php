...
<body class="bg-gray-50">
                                                    ...
                                                    <td class="px-6 py-4 whitespace-nowrap text-end text-sm font-medium">
                                                        <?php if (!in_array($transaction['payment_status'], ['paid', 'eft', 'paid_mixed'], true)): ?>
                                                            <div class="flex items-center justify-end gap-2">
                                                                <button type="button" class="inline-flex items-center gap-x-1.5 text-sm font-semibold rounded-lg border border-transparent text-green-600 hover:text-green-800 disabled:opacity-50 disabled:pointer-events-none dark:text-green-500 dark:hover:text-green-400 cash-payment-btn whitespace-nowrap" 
                                                                    data-sale-id="<?= $transaction['id'] ?>"
                                                                    data-balance="<?= $transaction['total_amount'] - $transaction['paid_amount'] ?>"
                                                                    title="Cash Payment">
                                                                    <i data-lucide="dollar-sign" class="w-4 h-4 shrink-0"></i>
                                                                    <span>Cash</span>
                                                                </button>
                                                                <button type="button" class="inline-flex items-center gap-x-1.5 text-sm font-semibold rounded-lg border border-transparent text-purple-600 hover:text-purple-800 disabled:opacity-50 disabled:pointer-events-none dark:text-purple-500 dark:hover:text-purple-400 eft-payment-btn whitespace-nowrap"
                                                                    data-sale-id="<?= $transaction['id'] ?>"
                                                                    data-balance="<?= $transaction['total_amount'] - $transaction['paid_amount'] ?>"
                                                                    title="EFT Payment">
    ...
    // Replace data-lucide placeholders with SVGs (icons are invisible until this runs)
