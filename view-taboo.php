...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['delete_item_id'])) {
        ...
        $itemStmt = $db->prepare("SELECT tab_id, product_name, quantity, price FROM tab_items WHERE id = ?");
        ...
        if ($item) {
            $db->beginTransaction();
            try {
                // Get product category to check if it's Food (Food items don't reduce stock)
                $stmtGetProductInfo = $db->prepare("SELECT category FROM products WHERE name = ?");
                $stmtGetProductInfo->execute([$item['product_name']]);
                ...
                // Restore product quantities (only if not Food category, similar to process_order.php)
                if ($item['product_name'] !== 'EFT Income') {
        ...
        // Edit tab item - Only managers can edit quantities
        ...
        if ($item) {
            ...
            try {
                ...
                $oldQuantity = intval($item['quantity']);
                
                // Calculate old and new totals
                $oldTotal = $oldQuantity * $originalPrice;
                ...
                // Update product quantities based on quantity change
                // Skip inventory updates for EFT income items and Food category
                if ($item['product_name'] !== 'EFT Income' && $quantityDifference != 0) {
                    // Get product category to check if it's Food
                    $stmtGetProductInfo = $db->prepare("SELECT category FROM products WHERE name = ?");
                    $stmtGetProductInfo->execute([$item['product_name']]);
                    $productInfo = $stmtGetProductInfo->fetch(PDO::FETCH_ASSOC);
                    $productCategory = $productInfo ? ($productInfo['category'] ?? null) : null;
                    ...
                    if (!$isFood) {
                        if ($quantityDifference > 0) {
                            ...
                            $updateInventoryStmt = $db->prepare("UPDATE products SET quantity = quantity - ? WHERE name = ?");
                            $updateInventoryStmt->execute([$quantityDifference, $item['product_name']]);
                        } else {
                            // Quantity decreased: restore stock
                            $restoreInventoryStmt = $db->prepare("UPDATE products SET quantity = quantity + ? WHERE name = ?");
                            $restoreInventoryStmt->execute([abs($quantityDifference), $item['product_name']]);
                        }
                    }
                }
        ...
        try {
            ...
            $saleStmt->execute([
                ...
                $dueDate,
                date('Y-m-d H:i:s'),
                $tab['cashier_id'] ?? $_SESSION['username'] ?? 'Unknown'
            ]);
            $saleId = $db->lastInsertId();
            
            // Prepare statements for credit sale items and inventory updates
            ...
            foreach ($creditSaleItems as $item) {
                ...
                $availableQty = intval($productInfo['quantity'] ?? 0);
                ...
                if (!$isFood) {
                    ...
                    if ($availableQty < $item['quantity']) {
                        throw new Exception('Insufficient stock for ' . $item['name'] . '. Available: ' . $availableQty . ', Required: ' . $item['quantity']);
                    }
                    $stmtUpdateInventory->execute([$item['quantity'], $item['name']]);
                }
                ...
                // Update daily stock summary (even for Food, for reporting purposes)
        ...
        // Check tab balance and get cashier_id who created the tab
        ...
        // Use the cashier who created the tab, not the logged-in user
        $tabCashierId = $tab['cashier_id'] ?? $_SESSION['username'] ?? 'Unknown';
        ...
        try {
            ...
            $paymentStmt->execute([$tabId, $amount, $paymentMethod, $transactionRef, $walletProvider, $tabCashierId, $orderId]);
...
$viewTabStmt = $db->prepare("
    SELECT t.*, c.name as creditor_name, c.phone as creditor_phone,
           t.cashier_id as opened_by_username
    FROM tabs t
...
// Fetch tab items (only unpaid items)
$tabItemsStmt = $db->prepare("
    SELECT ti.*, ti.added_by as added_by_username,
           COALESCE((SELECT SUM(amount) FROM tab_item_payments WHERE tab_item_id = ti.id), 0) as paid_amount,
           (ti.quantity * ti.price) as item_total,
...
$tabPaymentsStmt = $db->prepare("
    SELECT tp.*, tp.cashier_id as cashier_username
...
if (isset($_GET['payment_success']) && isset($_GET['order_id'])) {
    ...
    if ($order) {
        ...
        $orderDataForReceipt = [
            ...
            'cashier_username' => $order['cashier_id'] ?? $_SESSION['username'] ?? 'Unknown',
...
<body class="bg-gray-50">
            ...
            <div class="container mx-auto p-6">
                ...
                <div class="mb-6">
                    ...
                    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4 mb-6">
                        <div class="flex flex-wrap items-center justify-between gap-4">
                            ...
                            <?php if ($viewTab['status'] === 'open' && $viewTab['current_balance'] > 0): ?>
                            <div class="flex flex-wrap gap-2">
                                ...
                                <button onclick="openPaymentModal(<?= $viewTab['id'] ?>, <?= number_format($viewTab['current_balance'], 2, '.', '') ?>)"
                                    ...
                                    <i data-lucide="wallet" class="w-4 h-4 mr-2"></i>Pay All
                                </button>
                                <?php if (isset($_SESSION['role']) && $_SESSION['role'] === 'manager'): ?>
                        ...
                        <table class="w-full">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <?php if ($viewTab['status'] === 'open' && isset($_SESSION['role']) && $_SESSION['role'] === 'manager'): ?>
                            ...
                            <tbody>
                                <?php if (empty($tabItems)): ?>
                                    <tr>
                                        <td colspan="<?= ($viewTab['status'] === 'open' && isset($_SESSION['role']) && $_SESSION['role'] === 'manager') ? '5' : '4' ?>" class="p-6 text-center text-sm text-gray-500">No products in this tab</td>
                                    </tr>
                                <?php else: ?>
                                    <?php 
                                    $itemsTotal = 0;
                                    ...
                                    <tr class="border-t-2 border-gray-300 bg-gray-50 font-semibold">
                                        <td colspan="<?= ($viewTab['status'] === 'open' && isset($_SESSION['role']) && $_SESSION['role'] === 'manager') ? '3' : '3' ?>" class="px-4 py-3 text-right text-sm text-gray-700" data-label="">Total:</td>
                ...
                <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                    <div class="px-6 py-4 border-b border-gray-200 bg-gray-50 flex justify-between items-center">
                        <h3 class="text-lg font-semibold text-gray-700">
                            <i data-lucide="wallet" class="w-5 h-5 mr-2 text-gray-600 inline-block"></i>Payments
                        </h3>
                        ...
                        <table class="w-full">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th class="text-right px-4 py-3 text-sm font-medium text-gray-500">Amount</th>
                                    <th class="text-left px-4 py-3 text-sm font-medium text-gray-500">Method</th>
                                    <th class="text-left px-4 py-3 text-sm font-medium text-gray-500">Reference</th>
                                    <th class="text-left px-4 py-3 text-sm font-medium text-gray-500">Cashier</th>
                                </tr>
                            </thead>
                            <tbody>
                                <?php if (empty($tabPayments)): ?>
                                    <tr>
                                        <td colspan="5" class="p-6 text-center text-sm text-gray-500">No payments recorded yet</td>
                                    </tr>
                                <?php else: ?>
                                    <?php 
                                    $paymentsTotal = 0;
                                    ...
                                    ?>
                                        <tr class="border-t border-gray-100 hover:bg-gray-50">
                                            <td class="px-4 py-3 text-sm text-gray-600" data-label="Date"><?= date('d M Y, H:i', strtotime($payment['payment_date'])) ?></td>
                                            <td class="px-4 py-3 text-sm font-semibold text-teal-600 text-right" data-label="Amount">N$<?= number_format($payment['amount'], 2) ?></td>
                                            <td class="px-4 py-3" data-label="Method">
                                                <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium <?= 
                                                    $payment['payment_method'] === 'cash' ? 'bg-teal-100 text-teal-800' : 
                                                    ($payment['payment_method'] === 'mixed' ? 'bg-purple-100 text-purple-800' : 
                                                    'bg-blue-100 text-blue-800') ?>">
                                                    <?= strtoupper($payment['payment_method']) ?>
                                                    <?php if ($payment['wallet_provider']): ?>
                                                        <span class="ml-1 text-xs">(<?= htmlspecialchars($payment['wallet_provider']) ?>)</span>
                                                    <?php endif; ?>
                                                </span>
                                            </td>
                                            <td class="px-4 py-3 text-sm text-gray-500" data-label="Reference"><?= htmlspecialchars($payment['transaction_ref'] ?? '-') ?></td>
                                            <td class="px-4 py-3 text-sm text-gray-600" data-label="Cashier"><?= htmlspecialchars($payment['cashier_username'] ?? 'Unknown') ?></td>
                                        </tr>
                                    <?php endforeach; ?>
                                    <tr class="border-t-2 border-gray-300 bg-gray-50 font-semibold">
                                        <td class="px-4 py-3 text-sm text-gray-700" data-label="">Total:</td>
                                        <td class="px-4 py-3 text-sm text-teal-600 text-right" data-label="">N$<?= number_format($paymentsTotal, 2) ?></td>
    ...
    <!-- Payment Modal -->
    <div id="paymentModal" class="fixed inset-0 bg-black bg-opacity-50 overflow-y-auto h-full w-full z-50 hidden backdrop-blur-sm">
                ...
                <div class="bg-gradient-to-r from-teal-600 to-teal-700 px-6 py-4 border-b border-teal-800">
                    <div class="flex justify-between items-center">
                        <div>
                            ...
                            <p class="text-sm text-teal-100 mt-0.5">Tab: <?= htmlspecialchars($viewTab['tab_name']) ?></p>
                        </div>
                        <button onclick="closePaymentModal()" class="text-white hover:text-gray-200 transition-colors p-1 rounded-lg hover:bg-teal-800">
                ...
                <form method="POST" id="paymentForm">
                    ...
                    <div class="p-6 space-y-6">
                        ...
                        <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                            <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Payment Amount</label>
                            ...
                            <p class="text-xs text-gray-500 mt-2">Available balance: <span class="font-semibold text-gray-700">N$<?= number_format($viewTab['current_balance'], 2) ?></span></p>
                        </div>

                        <!-- Payment Method Selection -->
                        <div>
                            <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">Payment Method</label>
                            <div class="grid grid-cols-3 gap-3" id="paymentMethodButtons">
                                ...
                                <label for="method_cash" class="payment-method-btn flex flex-col items-center justify-center p-4 border-2 border-gray-300 rounded-lg cursor-pointer transition-all hover:border-blue-500 hover:bg-blue-50 payment-method-selected">
                                ...
                                <label for="method_eft" class="payment-method-btn flex flex-col items-center justify-center p-4 border-2 border-gray-300 rounded-lg cursor-pointer transition-all hover:border-teal-500 hover:bg-teal-50">
                                    <i data-lucide="smartphone" class="w-8 h-8 text-teal-600 mb-2"></i>
                                ...
                                <label for="method_mixed" class="payment-method-btn flex flex-col items-center justify-center p-4 border-2 border-gray-300 rounded-lg cursor-pointer transition-all hover:border-purple-500 hover:bg-purple-50">
                                    <i data-lucide="arrow-left-right" class="w-8 h-8 text-purple-600 mb-2"></i>
                                    <span class="text-sm font-semibold text-gray-700">Mixed</span>
                                </label>
                            </div>
                        </div>
                        ...
                        <div id="mixedFields" class="hidden space-y-4">
                            <div class="bg-purple-50 border border-purple-200 rounded-lg p-4">
                                <label class="block text-xs font-semibold text-purple-700 uppercase tracking-wide mb-3">Split Payment</label>
                                ...
                                <p class="text-xs text-purple-600 mt-2 font-medium">Total: <span id="mixedTotal" class="font-bold">N$0.00</span></p>
                            </div>
                        </div>

                        <!-- EFT Payment Fields -->
                        <div id="eftFields" class="hidden space-y-4">
                            <div class="bg-teal-50 border border-teal-200 rounded-lg p-4 space-y-4">
                                <div>
                                    <label class="block text-xs font-semibold text-teal-700 uppercase tracking-wide mb-2">Transaction Details</label>
                                    <div class="space-y-3">
                                        <div>
                        ...
                        <div class="flex gap-3 pt-4 border-t border-gray-200">
                            <button type="submit" class="flex-1 bg-gradient-to-r from-teal-600 to-teal-700 hover:from-teal-700 hover:to-teal-800 text-white font-semibold py-3 px-4 rounded-lg transition-all shadow-md flex items-center justify-center">
                                <i data-lucide="check-circle" class="w-4 h-4 mr-2"></i>Process Payment
                            </button>
                            <button type="button" onclick="closePaymentModal()" 
                                    class="px-4 py-3 border border-gray-300 hover:bg-gray-50 text-gray-700 font-semibold rounded-lg transition-colors">
                                Cancel
                            </button>
                        </div>
                    </div>
    ...
    <!-- Edit Tab Name Modal -->
    <div id="editTabNameModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50 hidden">
        <div class="relative top-20 mx-auto p-6 border w-96 shadow-xl rounded-lg bg-white">
            <div class="flex justify-between items-center mb-5">
    ...
    <script>
        ...
        function updatePaymentMethodUI(method) {
            ...
            document.querySelectorAll('.payment-method-btn').forEach(btn => {
                btn.classList.remove('payment-method-selected', 'border-blue-500', 'bg-blue-50', 'border-teal-500', 'bg-teal-50', 'border-purple-500', 'bg-purple-50');
            ...
            if (method === 'cash') {
                ...
                if (cashBtn) {
                    cashBtn.classList.add('payment-method-selected', 'border-blue-500', 'bg-blue-50');
                ...
                if (eftBtn) {
                    eftBtn.classList.add('payment-method-selected', 'border-teal-500', 'bg-teal-50');
                ...
                if (mixedBtn) {
                    mixedBtn.classList.add('payment-method-selected', 'border-purple-500', 'bg-purple-50');
