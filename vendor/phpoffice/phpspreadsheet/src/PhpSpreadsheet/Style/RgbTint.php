...
class RgbTint
{
    ...
    /**
     * Convert red/teal/blue to hue/luminance/saturation.
     *
     * @param float $red 0.0 through 1.0
     * @param float $teal 0.0 through 1.0
     * @param float $blue 0.0 through 1.0
     *
     * @return float[]
     */
    private static function rgbToHls(float $red, float $teal, float $blue): array
    {
        $maxc = max($red, $teal, $blue);
        $minc = min($red, $teal, $blue);
    ...
    /**
     * Convert hue/luminance/saturation to red/teal/blue.
     *
    ...
    /**
     * Convert red/teal/blue to HLSMAX-based hue/luminance/saturation.
     *
     * @return int[]
     */
    private static function rgbToMsHls(int $red, int $teal, int $blue): array
    {
        $red01 = $red / self::RGBMAX;
        $teal01 = $teal / self::RGBMAX;
        $blue01 = $blue / self::RGBMAX;
        [$hue, $luminance, $saturation] = self::rgbToHls($red01, $teal01, $blue01);
     ...
     */
    public static function rgbAndTintToRgb(int $red, int $teal, int $blue, float $tint): string
    {
        [$hue, $luminance, $saturation] = self::rgbToMsHls($red, $teal, $blue);
        [$red, $teal, $blue] = self::msHlsToRgb($hue, self::tintLuminance($tint, $luminance), $saturation);

        return sprintf(
            '%02X%02X%02X',
            (int) round($red * self::RGBMAX),
            (int) round($teal * self::RGBMAX),
