...
<script>
        ...
        function handleCashUp() {
            Swal.fire({
                title: '<h1 class="text-2xl font-bold text-teal-700 mb-4">Cash Up</h1>',
                html: `
                    <div class="space-y-4">
                        <label class="block text-sm font-medium text-gray-700 mb-2">Cash Up Amount:</label>
                        <input type="number" 
                               id="cashUpAmount" 
                               min="0" 
                               step="0.01" 
                               ...
                               placeholder="0.00">
                        <p class="text-xs text-gray-500">Additional cash received from customer</p>
                    </div>
                `,
                showCancelButton: true,
                reverseButtons: true,
                ...
                allowOutsideClick: false,
                preConfirm: () => {
                    const amount = parseFloat(document.getElementById('cashUpAmount').value);
                    if (!amount || amount <= 0) {
                        Swal.showValidationMessage('Please enter a valid cash up amount');
                        return false;
                    }
                    return { amount };
                }
                ...
                if (result.isConfirmed) {
                    ...
                    const cashUp = result.value.amount;
                    const newCashReceived = currentCash + cashUp;
                    ...
                    Swal.fire({
                        icon: 'success',
                        title: 'Cash Up Added',
                        text: `N$${cashUp.toFixed(2)} added to cash received`,
                        timer: 1500,
                        showConfirmButton: false
                    });
                }
                            ...
                            if (discount > 0 && discountStart && discountEnd) {
                                ...
                                if (now >= start && now <= end) {
                                    ...
                                    priceContainer.innerHTML = `
                                        ...
                                        <p class="text-sm mb-2 ${product.quantity < 5 ? 'text-red-600' : product.quantity < 10 ? 'text-yellow-600' : 'text-teal-600'}">Available: ${product.quantity}</p>
                                    `;
                                } else {
                                    priceContainer.innerHTML = `
                                        <p class="text-lg font-bold text-gray-900 whitespace-nowrap overflow-hidden text-ellipsis" title="${product.name}">${product.name}</p>
    ...
    document.addEventListener('DOMContentLoaded', refreshProductQuantities);
        ...
        function proceedWithTable(tableId, tableName = null) {
            ...
            // Final confirmation and optional print before processing tab sale
            Swal.fire({
                icon: 'success',
